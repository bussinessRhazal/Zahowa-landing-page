...
export default function FoundersForm() {
  ...
  const onSubmit = async (data) => {
    try {
      // Conversion multi-select arrays en string pour FormSubmit
      const formattedData = {
        ...data,
        moments: Array.isArray(data.moments) ? data.moments.join(', ') : data.moments,
        _subject: `🌟 Nouvelle inscription Cercle Fondateur — ${data.prenom}`,
        _template: 'table',
        _captcha: 'false'
      };
    ...
    return (
      <div className="founders-form founders-form--success">
        <div className="founders-form__success">
          ...
          <p className="founders-form__success-signature">
            — Abdelilah, Fès
          </p>
        </div>
      </div>
    ...
    <div className="founders-form">
      <header className="founders-form__header">
        ...
        <p className="founders-form__subtitle">
          Quelques questions pour façonner Zahowa avec toi.
        </p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <div className="founders-form__step-content">
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <select
                  className={`form-select ${errors.age ? 'has-error' : ''}`}
                  {...register('age', { required: 'Sélectionne ta tranche d\'âge' })}
                  defaultValue=""
                >
              ...
              <div className="form-group">
                ...
                <select
                  className={`form-select ${errors.genre ? 'has-error' : ''}`}
                  {...register('genre', { required: 'Sélectionne ton genre' })}
                  defaultValue=""
                >
                  <option value="" disabled>Choisir...</option>
            ...
            <div className="founders-form__actions">
              <button
                ...
                onClick={async () => {
                  // Valider étape 1 avant de passer à 2
                  const fields = ['prenom', 'email', 'age', 'genre'];
                  const isValid = await Promise.all(
                    fields.map((f) => new Promise((resolve) => {
                      const el = document.querySelector(`[name="${f}"]`);
                      resolve(el && el.value && el.value !== '');
                    }))
                  ).then((results) => results.every(Boolean));

                  if (isValid) setStep(2);
                  else handleSubmit(() => {})(); // Trigger validation display
                }}
              >
                Continuer
        ...
        {step === 2 && (
          <div className="founders-form__step-content">
                ...
                {FORM_OPTIONS.consumption.map((opt) => (
                  <label key={opt.value} className="form-radio">
                    <input
                      type="radio"
                      value={opt.value}
                      {...register('consommation', { required: 'Choisis une fréquence' })}
                    />
            ...
            <div className="form-group">
              <label className="form-label">À quels moments principalement ? <small>(plusieurs réponses possibles)</small></label>
            ...
            <div className="founders-form__actions founders-form__actions--split">
              ...
              <button
                ...
                onClick={async () => {
                  const consumption = document.querySelector('[name="consommation"]:checked');
                  if (consumption) setStep(3);
                  else handleSubmit(() => {})();
                }}
              >
                Continuer
        ...
        {/* ===== ÉTAPE 3 : Préférences Zahowa ===== */}
        {step === 3 && (
          <div className="founders-form__step-content">
            <p className="founders-form__step-label">Étape 3 sur 3 · Pour finir</p>

            <div className="form-group">
              <label className="form-label">Quel profil aromatique t'attire le plus ? *</label>
              <select
                className={`form-select ${errors.profil ? 'has-error' : ''}`}
                {...register('profil', { required: 'Sélectionne un profil' })}
                defaultValue=""
              >
                <option value="" disabled>Choisir...</option>
                ...
                {FORM_OPTIONS.intentions.map((opt) => (
                  <label key={opt.value} className="form-radio">
                    <input
                      type="radio"
                      value={opt.value}
                      {...register('intention', { required: 'Choisis une intention' })}
                    />
                    <span className="form-radio__mark"></span>
                    <span className="form-radio__label">{opt.label}</span>
                  </label>
            ...
            {submitStatus === 'error' && (
              <div className="form-error-banner">
                Une erreur est survenue. Réessaie ou écris-nous à <a href={`mailto:${SITE_CONFIG.contact.email}`}>{SITE_CONFIG.contact.email}</a>
              </div>
            )}
        ...
        <input type="hidden" name="_next" value={`${SITE_CONFIG.url}?inscription=success`} />
        <input type="hidden" name="_subject" value="🌟 Nouvelle inscription Cercle Fondateur Zahowa" />
